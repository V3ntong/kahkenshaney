...
const app = initializeApp({ projectId: PROJECT, apiKey: 'fake-key' });
const auth = getAuth(app);
connectAuthEmulator(auth, 'http://127.0.0.1:9099', { disableWarnings: true });
const storage = getStorage(app);
connectStorageEmulator(storage, '127.0.0.1', 9199);

const anonApp = initializeApp({ projectId: PROJECT, apiKey: 'fake-key' }, 'anon');
...
check('report photo, NO contentType (platform-inferred)',
  await upload('lost_and_found/lost/noct_1_0.jpg', { uploader: uid }), 'ALLOW');
check('report photo, octet-stream content type',
...
check('profile photo with NO contentType',
  await upload(`profiles/${uid}/posts/noct.jpg`, { uploader: uid }), 'ALLOW');
...
check('chat image over 10MB rejected',
  (async () => {
    ...
    catch (e) { return `DENY(${e.code ?? e.message})`; }
  })(), 'DENY');
...
check('delete a file without uploader metadata',
  await del('chat_images/c1.jpg'), 'DENY');
